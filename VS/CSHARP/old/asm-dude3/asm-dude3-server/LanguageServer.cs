...
public class LanguageServer
{
    ...
    private readonly DocumentHighlightProvider _documentHighlightProvider;
    private readonly AsmDude2Tools _asmDudeTools;
    private readonly MnemonicStore _mnemonicStore;
    private AsmLanguageServerOptions _options;
    ...
    public LanguageServer(ILogger logger)
    {
        ...
        _documentHighlightProvider = new DocumentHighlightProvider(logger);
    }
    ...
    public InitializeResult Initialize(int? processId, string? rootUri, object? capabilities, object? initializationOptions = null)
    {
        _logger.LogInformation("Received initialize request from client. ProcessId: {ProcessId}, RootUri: {RootUri}",
            processId, rootUri ?? "null");

        // Deserialize options from client if provided
        if (initializationOptions is Newtonsoft.Json.Linq.JToken jtoken)
        {
            try
            {
                _options = jtoken.ToObject<AsmLanguageServerOptions>() ?? CreateDefaultOptions();
                _logger.LogInformation("Loaded initialization options from client");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to deserialize initialization options, using defaults");
                _options = CreateDefaultOptions();
            }
        }
        else
    ...
    public SignatureHelp? GetSignatureHelp(SignatureHelpParams parameter)
    {
        _logger.LogDebug("Signature help requested for: {Uri} at line {Line}, character {Char}",
            parameter.TextDocument.Uri, parameter.Position.Line, parameter.Position.Character);
        ...
        if (extraLogging && parameter.Context != null)
        {
            ...
            _logger.LogInformation("GetSignatureHelp: TriggerKind={TriggerKind}; triggerChar={TriggerChar}; IsRetrigger={IsRetrigger}",
                parameter.Context.TriggerKind, parameter.Context.TriggerCharacter, parameter.Context.IsRetrigger);
        }

        // CRITICAL: Don't show signature help when semicolon is typed (comment start)
        if (parameter.Context?.TriggerCharacter == ";")
        {
            _logger.LogDebug("GetSignatureHelp: TriggerCharacter is semicolon, ignoring (comment)");
            return null;
        }

        var document = _documentManager.GetDocument(parameter.TextDocument.Uri);
        if (document == null)
        {
            _logger.LogWarning("Document not found: {Uri}", parameter.TextDocument.Uri);
            return null;
        }

        // Get the line text
        if (parameter.Position.Line < 0 || parameter.Position.Line >= document.Lines.Length)
        {
            _logger.LogWarning("Invalid line number: {Line}", parameter.Position.Line);
            return null;
        }

        var line = document.Lines[parameter.Position.Line];
        var charPosition = parameter.Position.Character;

        // Get signature help
        var signatureHelp = _signatureHelpProvider.ProvideSignatureHelp(line, charPosition, parameter.Position.Line);
    ...
    public DocumentHighlight[]? GetDocumentHighlights(DocumentHighlightParams parameter)
    {
        _logger.LogDebug("Document highlights requested for: {Uri} at line {Line}, character {Char}",
            parameter.TextDocument.Uri, parameter.Position.Line, parameter.Position.Character);
        ...
        var highlights = _documentHighlightProvider.ProvideDocumentHighlights(
            document.Lines,
            parameter.Position.Line,
            parameter.Position.Character);
        ...
        // Convert to LSP format
        return highlights.Select(h => new DocumentHighlight
        {
            Range = new Range
            {
                Start = new Position { Line = h.Range.StartLine, Character = h.Range.StartChar },
                End = new Position { Line = h.Range.EndLine, Character = h.Range.EndChar }
            },
            Kind = (int)h.Kind
        }).ToArray();
    }
    ...
    {
        _logger.LogDebug("Find references requested for: {Uri} at line {Line}, character {Char}",
            parameter.TextDocument.Uri, parameter.Position.Line, parameter.Position.Character);
        ...
        // Reuse DocumentHighlightProvider logic to find all occurrences
        var highlights = _documentHighlightProvider.ProvideDocumentHighlights(
            document.Lines,
            parameter.Position.Line,
            parameter.Position.Character);
        ...
        return highlights.Select(h => new Location
        {
